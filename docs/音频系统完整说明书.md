...
| 异步 Worker | `screening-service/app/workers/pipeline.py` | 转录、特征、LLM 分阶段消费、重试和结果发布 |
| 分析产物 | `screening-service/app/artifacts/store.py` | 原子写入阶段 JSON、`analysis.json` 和 SHA-256 |
...
当前异步筛查按独立 RabbitMQ 队列执行：

```text
转录 Worker：路径约束 → WAV 质量检查 → Whisper → transcription-attempt-{n}.json
→ transcription.completed
→ 特征 Worker：MFCC + 语言特征 → features-attempt-{n}.json
→ features.completed
→ LLM Worker：DeepSeek 报告
→ 原子写 analysis.json
...
每个阶段设置 `prefetch_count=1` 且手动 ACK。转录阶段交棒后立即领取下一条音频；阶段产物按 task attempt 隔离，自动投递重试不会改变业务 attempt。
...
data/screening-artifacts/{taskId}/transcription-attempt-{n}.json
data/screening-artifacts/{taskId}/features-attempt-{n}.json
```
...
- 在异步模式下检查 RabbitMQ，并启动 `transcription`、`features`、`llm` 三类流水线 Worker；
...
- 多转录 Worker 时从基础 GPU 索引起给各 Worker 分配索引；特征和 LLM Worker 不占用 Whisper GPU。

转录 Worker 启动时会调用 `get_whisper_model()`，在开始消费 RabbitMQ 消息前把 Whisper 模型载入对应 GPU。

### 20.3 当前是否预热音频模型

异步转录 Worker 现在会在连接 RabbitMQ、开始消费任务之前调用 `get_whisper_model()`，因此模型下载和 GPU 装载发生在 Worker 启动阶段，而不是第一条用户音频。同步 `app.main` 的 `/api/diagnosis` 路径仍然按首次调用懒加载。

当前预热只加载模型，没有使用用户音频执行推理。若还希望预热 CUDA 推理内核，可以后续加入随代码提供的短静音测试 WAV。预热需考虑：
