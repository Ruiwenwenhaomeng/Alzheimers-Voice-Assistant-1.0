...
function updateActiveModelBadge() {
  ...
  if (provider === "SYSTEM") { $("activeModelBadge").textContent = "系统默认"; return; }
...
function saveAssistantModelSettings(event) {
  ...
  try {
    if (provider === "SYSTEM") state.assistantModelProfile.activeProvider = "SYSTEM";
    else {
    ...
    persistAssistantModelSettings(); updateActiveModelBadge();
    setStatus($("modelSettingsStatus"), "模型设置已保存，下一次提问立即生效。", "success");
  } catch (error) { setStatus($("modelSettingsStatus"), `保存失败：${error.message}`, "error"); }
}
...
function clearSelectedModelApiKey() {
  ...
  if (state.assistantModelProfile.activeProvider === provider) state.assistantModelProfile.activeProvider = "SYSTEM";
  $("modelApiKey").value = ""; persistAssistantModelSettings(); updateActiveModelBadge();
...
function renderAssistantWelcome() {
  ...
  appendAssistantMessage({
    title: "你好，我是忆声助手",
    answer: "我可以解释常见表现、语音筛查、就医评估、风险管理、治疗常识和家庭照护。"
  });
}
...
function renderConversationList() {
  ...
  state.assistantConversations.forEach((conversation) => {
    ...
    select.title = `${conversation.title}（${conversation.turnCount}/${conversation.maxTurns}轮）`;
...
async function initializeAssistantConversations() {
  ...
  try {
    ...
    if (!conversations.length) {
      const created = await assistantApi("/assistant/conversations", {
    ...
    await selectConversation(conversations[0].id, version);
  } catch (error) {
    if (version !== state.assistantLoadVersion) return;
...
async function createConversation() {
  ...
  try {
    const conversation = await assistantApi("/assistant/conversations", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: "{}"
    });
    ...
    appendAssistantMessage({ title: "无法新建对话", answer: error.message });
  }
}
...
async function deleteConversation(id) {
  ...
  try {
    ...
    if (!state.assistantConversations.length) {
      ...
      await selectConversation(state.assistantConversations[0].id);
    } else {
      renderConversationList();
    }
  } catch (error) {
    appendAssistantMessage({ title: "无法删除对话", answer: error.message });
  }
}
...
async function selectConversation(id, expectedVersion = state.assistantLoadVersion) {
  ...
  try {
    ...
    renderConversationList();
    updateConversationHeader(detail.conversation);
    $("chatMessages").replaceChildren();
    ...
    detail.messages.forEach((message) => {
      ...
      else appendAssistantMessage({ ...message, answer: message.content });
    });
  } catch (error) {
    appendAssistantMessage({ title: "对话加载失败", answer: error.message });
  }
...
async function askQuestion(question) {
  ...
  if (active && active.turnCount >= active.maxTurns) {
    appendAssistantMessage({ title: "本对话已满", answer: "每个对话最多 100 轮，请点击“新对话”继续。" });
    return;
  }
  ...
  const title = document.createElement("h3");
  title.textContent = "正在生成回答…";
  const answer = document.createElement("p");
  answer.className = "streaming-cursor";
  placeholder.append(title, answer);
  ...
  try {
    const response = await fetch(`/assistant/conversations/${encodeURIComponent(state.currentConversationId)}/messages/stream`, {
      method: "POST",
      headers: assistantHeaders({ "Content-Type": "application/json", "Accept": "text/event-stream" }),
      body: JSON.stringify({ message: value, modelSettings })
    });
    ...
    while (true) {
      ...
      for (const block of blocks) {
        ...
        if (message.event === "delta") {
          answer.textContent += message.data.content || "";
          $("chatMessages").scrollTop = $("chatMessages").scrollHeight;
        } else if (message.event === "complete") {
          completed = true;
          appendAssistantMessage(message.data.response, placeholder);
          const conversation = message.data.conversation;
          const index = state.assistantConversations.findIndex((item) => item.id === conversation.id);
          if (index >= 0) state.assistantConversations[index] = conversation;
          state.assistantConversations.sort((a, b) => a.id === conversation.id ? -1 : b.id === conversation.id ? 1 : 0);
          updateConversationHeader(conversation);
          renderConversationList();
        } else if (message.event === "error") {
          throw new Error(message.data.message || "回答生成失败");
        }
      }
      if (done) break;
    }
    ...
    appendAssistantMessage({ title: "回答中断", answer: error.message }, placeholder);
  } finally {
    state.chatStreaming = false;
    const current = state.assistantConversations.find((item) => item.id === state.currentConversationId);
    updateConversationHeader(current);
  }
}
...
async function runScreening(audioName, button) {
  button.disabled = true;
  setStatus($("screeningStatus"), "正在进行转写、质量检查和风险分析，请耐心等待...");
  try {
    const result = await api(`/audio/screening/${encodeURIComponent(audioName)}`, { method: "POST" }, true);
    state.currentDiagnosis = { audioName, ...result };
    renderResult(result);
    renderDiagnosisPreview(state.currentDiagnosis);
    $("savePdf").disabled = false;
    setStatus($("screeningStatus"), "分析完成。请结合医疗边界阅读结果。", "success");
...
async function runDiagnosis() {
  ...
  setStatus($("diagnosisStatus"), "正在生成诊断报告...");
  try {
    const check = await api(`/audio/diagnosis/check/${encodeURIComponent(fileName)}`, {}, true);
    if (check.alreadyChecked && !window.confirm("该录音已经生成过诊断报告，仍要重新生成吗？")) {
      return;
    }
    const result = await api(`/audio/diagnosis/${encodeURIComponent(fileName)}`, {}, true);
    state.currentDiagnosis = { audioName: fileName, ...result };
    renderDiagnosisPreview(state.currentDiagnosis);
    renderResult(result);
    $("savePdf").disabled = false;
    setStatus($("diagnosisStatus"), "诊断报告已生成。", "success");
    await loadPdfList();
  } catch (error) {
